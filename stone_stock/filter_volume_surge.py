...
class VolumeSurgeFilter:
    ...
    def check_conditions(self, df):
        ...
        # 判断是否符合条件
        meets_conditions = all(volume_grows)
        ...
        if meets_conditions:
            ...
            info = {
                ...
                'latest_volume': recent_data.loc[self.days, 'volume'],
                'total_price_change_pct': ((recent_data.loc[self.days, 'close'] - recent_data.loc[0, 'close']) 
                                          / recent_data.loc[0, 'close'] * 100),
                'avg_volume_growth': sum(volume_growth_rates) / len(volume_growth_rates)
            }
            return True, info
    ...
    def filter_stocks(self, stock_list):
        ...
        print("=" * 100)
        print(f"筛选条件：")
        print(f"  ✓ 最近 {self.days} 天成交量连续增长≥{self.volume_growth}%")
        print(f"  ✓ 不限制价格涨跌（成交量放大即可）")
        print()
        ...
        for idx, row in stock_list.head(total).iterrows():
            ...
            if meets:
                ...
                up_down = "↑" if info['total_price_change_pct'] > 0 else "↓" if info['total_price_change_pct'] < 0 else "→"
                print(f"✓ 找到: {stock_code} {stock_name} (成交量平均增长{info['avg_volume_growth']:.1f}%, 价格{info['total_price_change_pct']:+.2f}% {up_down})")
    ...
    def display_results(self):
        ...
        sorted_stocks = sorted(self.qualified_stocks, 
                              key=lambda x: x['avg_volume_growth'], 
                              reverse=True)
        
        for i, stock in enumerate(sorted_stocks, 1):
            up_down = "⬆️" if stock['total_price_change_pct'] > 0 else "⬇️" if stock['total_price_change_pct'] < 0 else "➡️"
            
            print(f"【{i}】{stock['code']} - {stock['name']}")
            print(f"    最新价格: {stock['latest_close']:.2f} 元")
            print(f"    {self.days}日价格变化: {stock['total_price_change_pct']:+.2f}% {up_down}")
            print(f"    平均成交量增长: {stock['avg_volume_growth']:.1f}%")
            ...
            # 显示每日价格变化
            price_change_str = ' → '.join([f"{p:+.2f}%" for p in stock['price_changes']])
            print(f"    每日涨跌: {price_change_str}")
            
            print()
    
    def save_results(self, filename='volume_surge_stocks'):
        """保存结果"""
        if not self.qualified_stocks:
            return
        ...
        for stock in self.qualified_stocks:
            row = {
                ...
                f'{self.days}日价格变化(%)': round(stock['total_price_change_pct'], 2),
                '平均成交量增长(%)': round(stock['avg_volume_growth'], 2),
            }
            
            # 添加每日数据
            for i, (date, close, vol_growth, price_change) in enumerate(zip(
                stock['dates'], stock['closes'], stock['volume_growth_rates'], stock['price_changes']), 1):
                row[f'第{i}天日期'] = date
                row[f'第{i}天收盘'] = close
                row[f'第{i}天成交量增长(%)'] = round(vol_growth, 2)
                row[f'第{i}天涨跌(%)'] = round(price_change, 2)
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='股票筛选工具 - 成交量放大版')
    ...
    parser.add_argument('-n', '--number', type=int, default=None,
                       help='扫描股票数量（默认全部）')
    parser.add_argument('-o', '--output', type=str, default='volume_surge_stocks',
                       help='输出文件名（默认volume_surge_stocks）')
    ...
    print()
    print("特点:")
    print(f"  ✓ 只关注成交量放大，不限制价格涨跌")
    print(f"  ✓ 可能捕捉到大资金进场信号")
    print(f"  ✓ 适合寻找潜在热点")
    print()
    
    # 创建筛选器
    filter = VolumeSurgeFilter(
        days=args.days,
